...
from auth_user_.user_perm import IsUser
...
class AddToCard(APIView):
    ...
    def post(self, request):
        ...
        if not Gullar.objects.filter(id=product_id).exists():
            data = {
                'error':"siz mavjud bolmagan kitobdi tanladiz",
        ...
        gullar = Gullar.objects.get(id=product_id)


        if not CardItem.objects.filter(card=card, product=gullar).exists():
            gullar = CardItem.objects.create(
               card = card,
               product = gullar,
               amount = amount 
            )
        else:
            gullar = CardItem.objects.get(card=card, product=gullar)
            gullar.amount += amount

        gullar.save()

        serializer = CardItemsSerializer(gullar)
        data = {
